...
public class Menu {
	...
	/**
	 * creates game with inserted parameters
	 * pre:
	 * pos: starts the program
	 * @throws IOException
	 */
	public void createGameZone() throws IOException {
		...
		// Getting amount of cells in the board, to verify if the amount of snakes and
		// ladders is coherent
		int size1 = Integer.parseInt(initialData[0]) * Integer.parseInt(initialData[1]);
		boardC = new BoardControl();
		// i multiply every snakes and ladders value by 2 because each one has a start
		// and an end
		int snakesAndLaddersValidation = (Integer.parseInt(initialData[2]) * 2)
				+ (Integer.parseInt(initialData[3]) * 2);
		if (snakesAndLaddersValidation < size1) {
			if (Integer.parseInt(initialData[1]) <= 26 && Integer.parseInt(initialData[2]) <= size1
					&& Integer.parseInt(initialData[3]) <= size1) {
				startBoard(size1);
				strtPlayers(size1, initialData[4]);
				addEol(Integer.parseInt(initialData[0]), Integer.parseInt(initialData[1]),
						Integer.parseInt(initialData[3]), 49);
				addSol(Integer.parseInt(initialData[0]), Integer.parseInt(initialData[1]),
						Integer.parseInt(initialData[3]), 49);
				addEos(Integer.parseInt(initialData[0]), Integer.parseInt(initialData[1]),
						Integer.parseInt(initialData[2]), 65);
				addSos(Integer.parseInt(initialData[0]), Integer.parseInt(initialData[1]),
						Integer.parseInt(initialData[2]), 65);
				createPlayers(initialData[4], playerAmount - 1);
				playGame(Integer.parseInt(initialData[0]), Integer.parseInt(initialData[1]));
				play(0, playerAmount, Integer.parseInt(initialData[0]), Integer.parseInt(initialData[1]),
						Integer.parseInt(initialData[2]), Integer.parseInt(initialData[3]), false);
			}
		} else {

			System.out.println(
					"La cantidad de serpientes y/o escaleras es demasiada para el tamaño del leaderBoardro, porfavor ingrese nuevamente");
			createGameZone();
		}
	}
	...
	public void play(int turn, int amountPlayers, int row, int column, int snake, int ladders, boolean simulationMode)
			throws IOException {
			...
			if (turn < amountPlayers) {
				...
				if (next.isEmpty()) {
					...
					if (boardC.movePlayers(player, turn, dice)) {
						...
						leaderBoard.startGame(boardC.allMvmnts(boardC.player(turn)) * (row * column), column, row,
								snake, ladders, amountPlayers, player);// binary three
						System.out.println("Player: " + realName + "\n" + "Score: "
								+ boardC.allMvmnts(boardC.player(turn)) * (row * column) + "\n");
						startprogram();
					} else {
						System.out.println(boardC.showContent(row, column));
			...
			if (turn < amountPlayers) {
				...
				if (next.isEmpty()) {
					...
					if (boardC.movePlayers(player, turn, dice)) {
						...
						String realName = br.readLine();
						leaderBoard.startGame(boardC.allMvmnts(boardC.player(turn)) * (row * column), column, row,
								snake, ladders, amountPlayers, player);// calling binary three
						System.out.println("Player: " + realName + "\n" + "Score: "
								+ boardC.allMvmnts(boardC.player(turn)) * (row * column) + "\n");
						startprogram();
					} else {

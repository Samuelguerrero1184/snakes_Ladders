	...
	private String show(int col, int row, Board t, int i) {
		...
		if (row != 0) {
			...
			if (col >= 1) {
				if (x == 0) {
					i++;
					leaderBoard = leaderBoard + "[ " + t.getSize() + t.getLadders() + t.getSnakes() + "]";

					return show(col = col - 1, row, t.getNextMvmnt(), i);
				} else {
					i++;
					int n = t.getSize() - col + i;
					leaderBoard = leaderBoard + "[ " + readBoard(n).getSize() + readBoard(n).getLadders()
							+ readBoard(n).getSnakes() + "]";
		...
		if (row != 0) {
			...
			if (col >= 1) {
				if (x == 0) {
					...
					i++;
					int n = t.getSize() - col + i;
					leaderBoard2 = leaderBoard2 + "[" + readBoard(n).getLadders() + readBoard(n).getSnakes()
							+ readBoard(n).getPlayers() + "]";
					return showColumns(col = col - 1, row, t.getNextMvmnt(), i);
				}
			} else {
					...
					if (valLadder(readBoard(newPos))) {
						...
						if (valSnake(readBoard(newPos))) {
							...
							if (t.getPlayers() != "") {
								...
								t.setPlayers(t.getPlayers().replace(t.getPlayers(),
										t.getPlayers() + String.valueOf(player.getPlayer())));
								return false;
							} else {
	...
	private void moveAnotherSnake(Board snake, Player player) {
		...
		if (encounterSnake.getSize() == snake.getSize()) {
			...
			if (encounterSnake.getPlayers() != "") {
				...
				encounterSnake.setPlayers(encounterSnake.getPlayers().replace(encounterSnake.getPlayers(),
						encounterSnake.getPlayers() + String.valueOf(player.getPlayer())));
			} else {
			...
			if (snake.getPlayers() != "") {
				...
				snake.setPlayers(snake.getPlayers().replace(snake.getPlayers(),
						snake.getPlayers() + String.valueOf(player.getPlayer())));
			} else {
		...
		if (encounterLadder.getSize() != ladder.getSize()) {
			if (encounterLadder != start) {
				if (encounterLadder.getPlayers() != "") {
					encounterLadder.setPlayers(encounterLadder.getPlayers().replace(encounterLadder.getPlayers(),
							encounterLadder.getPlayers() + String.valueOf(player.getPlayer())));
					return false;
				} else {
			...
			if (ladder.getPlayers() != "") {
				ladder.setPlayers(ladder.getPlayers().replace(ladder.getPlayers(),
						ladder.getPlayers() + String.valueOf(player.getPlayer())));
				return false;
			} else {
